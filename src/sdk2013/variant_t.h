...
#define _SIGSEGV_SDK2013_OVERRIDE__PUBLIC_VARIANT_T_H 1
...
public:
	// Dumb comparison
	bool operator==(variant_t const& other) const { return fieldType == other.fieldType && (fieldType == FIELD_VECTOR || fieldType == FIELD_POSITION_VECTOR ? vecVal[0] == other.vecVal[0] && vecVal[1] == other.vecVal[1] && vecVal[2] == other.vecVal[2] : iVal == other.iVal); }
	...
	void SetEntity( EHANDLE val ) { eVal = val, fieldType = FIELD_EHANDLE; }
	...
	void SetRaw32( fieldtype_t ftype, void *data ) { fieldType = ftype; iVal = *(int *)data; }
	void SetRaw( fieldtype_t ftype, void *data ) { fieldType = ftype; vecVal[0] = *(float *)data; vecVal[1] = *((float *)data+1); vecVal[2] = *((float *)data+2); }
